...
function creerCarteProduit(produit) {
    ...
    return `
        <div class="produit-card ${enStockClass}" onclick="ouvrirModal(${produit.id})">
            ...
            <button class="btn btn-secondary" onclick="event.stopPropagation(); openWhatsApp('${produit.nom}', ${produit.prix})">Commander</button>
        </div>
    `;
}

// ========== OUVERTURE WHATSAPP ==========
function openWhatsApp(nom, prix) {
    const numero = "243972685669"; // +243 972 685 669 sans espaces ni +
    const message = `Bonjour, je suis intéressé par le produit *${nom}* au prix de *${prix}$*. Pouvez-vous me donner plus d'informations ?`;
...
const modalCommanderBtn = document.getElementById('modalCommanderBtn');
...
function ouvrirModal(produitId) {
    ...
    // Attacher l'événement au bouton Commander de la modal
    if (modalCommanderBtn) {
        modalCommanderBtn.onclick = function() {
            openWhatsApp(produit.nom, produit.prix);
        };
    }
